...
public class BasketOperations 
{
	...
	private static HashMap<String,JSONObject> getISINStockMap(JSONArray cons ) throws Exception
	{
		...
		try 
		{
			...
			st=conn.prepareStatement("select 'NSE' as EXCHANGE,ISIN,SYMBOL from NSE_QUOTE where ISIN in ("+consStr.toString()+") union all select 'BSE' as exch,ISIN,SYMBOL from BSE_QUOTE where ISIN in ("+consStr.toString()+")");
			res=st.executeQuery();
	...
	public static GetBasketDetailsResponse getBasketDetails(GetBasketDetailsRequest bdReq) throws Exception
	{
		...
		try {
			...
			String sQuery="select bc.EXCHANGE,bc.QTY,bc.QTY*("+queries[0]+") as PRICE, "+queries[0].replace("LAST_PRICE", "SYMBOL_NAME")
					+" as SYMBOL_NAME from BASKET_CONSTITUENTS bc "+queries[1]+" where bc.BASKET_NAME=? and bc.IS_LATEST=1";
			pstmt = conn.prepareStatement(sQuery);
			pstmt.setString(1, basketName);
			log.debug(pstmt);
	...
	{		
		...
		if(exchList.size()>1)
		{
			...
			for(String exch:exchList)
			{
				...
				sb1.append(" left join quote_data."+table+tempTable+" on bc.EXCHANGE='"+exch+"' and bc.SYMBOL="+tempTable+".SYMBOL");
			}
	...
	public static BasketResponse modifyBasket(ModifyBasketRequest modifyReq) throws Exception
	{
		...
		try 
		{
			...
			if(investType>=0)updateColums.append("INVEST_TYPE="+investType+",");
			if(initialFee!=null)updateColums.append("INITIAL_FEE="+initialFee+",");
			if(rebalanceFee!=null)updateColums.append("REBALANCE_FEE="+rebalanceFee+",");
			if(monitorFee!=null)updateColums.append("MONITOR_FEE="+monitorFee+",");
			if(trendingFlag>=0)updateColums.append("TRENDING_FLAG="+trendingFlag+",");
			if(recommendedFlag>=0)updateColums.append("RECOMMENDED_FLAG="+recommendedFlag+",");
			if(featuredFlag>=0)updateColums.append("FEATURED_FLAG="+featuredFlag+",");
			if(riskFlag>=0)updateColums.append("RISK_FLAG="+riskFlag+",");		
			if(!strength.isEmpty())updateColums.append("STRENGTH="+strength+",");
			if(!valueForMoney.isEmpty())updateColums.append("VALUE_FOR_MONEY="+valueForMoney+",");

			if(!category.isEmpty())updateColums.append("CATEGORY='"+category+"',");
			if(!subCategory.isEmpty())updateColums.append("SUB_CATEGORY='"+subCategory+"',");		
			if(!description.isEmpty())updateColums.append("DESCRIPTION='"+fromHtmlTOString(description)+"',");
			if(!rationale.isEmpty())updateColums.append("RATIONALE='"+fromHtmlTOString(rationale)+"',");
			if(!disclaimer.isEmpty())updateColums.append("DISCLAIMER='"+fromHtmlTOString(disclaimer)+"',");		
			if(indexValue!=null)updateColums.append("INDEX_VALUE="+indexValue+",");
			if(!benchMark.isEmpty())updateColums.append("BENCHMARK='"+benchMark+"',");
			if(!imageURL.isEmpty())updateColums.append("IMAGE_URL='"+imageURL+"',");
			if(!iconURL.isEmpty())updateColums.append("ICON_URL='"+iconURL+"',");
			if(oneMonthRet!=null)updateColums.append("ONE_MONTH_RET="+oneMonthRet+",");
			if(threeMonthRet!=null)updateColums.append("THREE_MONTH_RET="+threeMonthRet+",");
			if(sixMonthRet!=null)updateColums.append("SIX_MONTH_RET="+sixMonthRet+",");
			if(oneYearRet!=null)updateColums.append("ONE_YEAR_RET="+oneYearRet+",");
			if(threeYearRet!=null)updateColums.append("THREE_YEAR_RET="+threeYearRet+",");
			if(fiveYearRet!=null)updateColums.append("FIVE_YEAR_RET="+fiveYearRet+",");
