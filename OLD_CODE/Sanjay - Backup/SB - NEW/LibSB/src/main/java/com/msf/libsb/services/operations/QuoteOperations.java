	...
	public static QuoteOverviewResponse quoteOverView(String basketName, String userId) throws Exception {
		...
		try {
			...
			if (res.next()) {
				...
				resp.setPopularity(FetchBasketOperations.popularityArr[popularity]);
				resp.setStrength(FetchBasketOperations.strengthArr[strength]);
				resp.setValueForMoney(FetchBasketOperations.valueForMoneyArr[valueForMoney]);
