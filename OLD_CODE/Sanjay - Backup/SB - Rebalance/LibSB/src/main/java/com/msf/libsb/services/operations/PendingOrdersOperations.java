	...
	public static GetPendingOrdersResponse getBrokenOrders(String userId) throws Exception {
		...
		try {
			...
			while (res.next()) {
				...
				order.put(APP_CONSTANT.PRICE, SamcoHelper.getIndianCurrencyFormat(res.getDouble(DBConstants.PRICE)));
	...
	public static GetPendingOrdersResponse getRebalanceOrders(String userId) throws Exception {
		...
		try {
			...
			while (res.next()) {
				...
				// order.put(APP_CONSTANT.BASKET_NAME,
				// res.getString(DBConstants.BASKET_NAME));
				// order.put(APP_CONSTANT.ICON_URL,
				// res.getString(DBConstants.ICON_URL));
				//
				// if (isLatest) {
				// order.put(APP_CONSTANT.PENDING_SINCE,
				// res.getDate(DBConstants.CREATED_AT));
				...
				// log.debug("----> BASKET_NAME :: " +
				// res.getString(DBConstants.BASKET_NAME) + " :: " + orderNo);
				// if
				// (orderMap.containsKey(res.getString(DBConstants.BASKET_NAME)))
				// {
				// JSONObject tempObject = (JSONObject)
				// orderMap.get(res.getString(DBConstants.BASKET_NAME));
				...
				// order.put(APP_CONSTANT.PENDING_SINCE,
				// res.getDate(DBConstants.CREATED_AT));
				// order.put(APP_CONSTANT.PRICE,
				// SamcoHelper.getIndianCurrencyFormat(price + existingPrice));
				// } else {
				// order.put(APP_CONSTANT.PRICE,
				// SamcoHelper.getIndianCurrencyFormat(existingPrice - price));
				// }
				// }
				// }

				String basketName = res.getString(DBConstants.BASKET_NAME);
				String iconUrl = res.getString(DBConstants.ICON_URL);
				String orderNo = res.getString(DBConstants.ORDER_NO);
				double price = res.getDouble(DBConstants.PRICE);
				boolean isLatest = res.getBoolean(DBConstants.IS_LATEST);
				Date createdAt = res.getDate(DBConstants.CREATED_AT);

				JSONObject row = new JSONObject();

				if (orderMap.containsKey(basketName)) {
					row = orderMap.get(basketName);
					if (isLatest) {

						JSONArray arr = row.getJSONArray("orderArr");
						arr.put(orderNo);
						row.put("orderArr", arr);

						row.put(APP_CONSTANT.CREATED_AT, createdAt);
					} else {
						price = -price;
					}
					double sum = Double.valueOf(String.valueOf(row.get(APP_CONSTANT.PRICE)).replaceAll(",", ""))
							+ price;
					row.put(APP_CONSTANT.PRICE, SamcoHelper.getIndianCurrencyFormat(sum));

				} else {

					JSONArray orderArray = new JSONArray();

					row.put(APP_CONSTANT.BASKET_NAME, basketName);
					row.put(APP_CONSTANT.ICON_URL, iconUrl);
					row.put(APP_CONSTANT.CREATED_AT, createdAt);
					row.put(APP_CONSTANT.PRICE, -price);
					row.put("orderArr", orderArray.put(orderNo));

				}
				orderMap.put(basketName, row);
