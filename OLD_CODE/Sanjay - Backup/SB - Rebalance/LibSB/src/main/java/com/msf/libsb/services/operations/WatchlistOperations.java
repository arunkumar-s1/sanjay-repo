		...
		try {
			...
			while (res.next()) {
				...
				log.debug("changePer-------->" + changePer);
								...
								basket.put(APP_CONSTANT.STRENGTH, FetchBasketOperations.strengthArr[strength]);
				basket.put(APP_CONSTANT.VALUE_FOR_MONEY,
						FetchBasketOperations.valueForMoneyArr[valueForMoney]);
				basket.put(APP_CONSTANT.POPULARITY,
						FetchBasketOperations.popularityArr[popularity]);
				basket.put(APP_CONSTANT.INDEX_VALUE, res.getString(DBConstants.INDEX_VALUE));
