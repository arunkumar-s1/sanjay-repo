...
public class Save2FaInDBRequest extends BaseRequest
{
	...
	public BaseResponse postRequest() throws Exception 
	{
		...
		try
		{
			...
			String sQuery = "update samco.USERS_2FA set answer = ? where user_id = '"+userId+"' and value = ?";
			...
			sQuery = "delete from samco.USERS_2FA where user_id = ? and value not in (" + sb.toString() + ")";
			ps = conn.prepareStatement(sQuery);
