...
//	private static Logger log = Logger.getLogger(QuoteOverview.class);
	...
	protected void doPostProcessRequest(SSRequest ssRequest, SSResponse ssResponse) throws Exception {
		...
		ssResponse.addToData(APP_CONSTANT.NET_PRICE,
				SamcoHelper.getIndianCurrencyFormat(orderReviewResponse.getNetPrice()));
		if (orderReviewResponse.getTransactionType().equalsIgnoreCase(APP_CONSTANT.BUY)) {
			ssResponse.addToData(APP_CONSTANT.ESTIMATED_TAX,
					SamcoHelper.getIndianCurrencyFormat(orderReviewResponse.getEstimatedTax()));
			ssResponse.addToData(APP_CONSTANT.ESTIMATED_FEE,
					SamcoHelper.getIndianCurrencyFormat(orderReviewResponse.getEstimatedFees()));
			ssResponse.addToData(APP_CONSTANT.NET_PRICE,
					SamcoHelper.getIndianCurrencyFormat(orderReviewResponse.getNetPrice()));
			
		} else {
			Double dTotalTax = qty * orderReviewResponse.getEstimatedFees();
			ssResponse.addToData(APP_CONSTANT.ESTIMATED_TAX,
					SamcoHelper.getIndianCurrencyFormat(dTotalTax));
			ssResponse.addToData(APP_CONSTANT.ESTIMATED_FEE,
					SamcoHelper.getIndianCurrencyFormat(orderReviewResponse.getEstimatedTax()));
			Double dTotalPrice = orderReviewResponse.getTotalPrice() + orderReviewResponse.getEstimatedFees() - dTotalTax;
			orderReviewResponse.setTotalPrice(dTotalPrice);
		}
		ssResponse.addToData(APP_CONSTANT.TOTAL_PRICE,
				SamcoHelper.getIndianCurrencyFormat(orderReviewResponse.getTotalPrice()));
		ssResponse.addToData(APP_CONSTANT.MIN_AMT,
				SamcoHelper.getIndianCurrencyFormat(orderReviewResponse.getMinAmount()));
	}
