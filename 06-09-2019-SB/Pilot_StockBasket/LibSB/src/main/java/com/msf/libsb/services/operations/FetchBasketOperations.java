...
public class FetchBasketOperations {
	...
	public static FetchBasketsResponse fetchBaskets(FetchBasketsRequest fetchBaskets) throws Exception {
		...
		try {
			...
			while (res.next()) {
				...
				if (!res1.next()) {
					...
					do {
						log.debug("Value is :: " + res1.getDouble(DBConstants.INDEX_VALUE));
						if (!bFirstValue) {
							log.debug("First if");
				...
				String indexValue = dec.format(dFirstValue);
				indexList.clear();
				...
				basket.put(APP_CONSTANT.STRENGTH, strengthArr[strength == 0 ? 0 : strength]);
				basket.put(APP_CONSTANT.VALUE_FOR_MONEY, valueForMoneyArr[valueForMoney == 0 ? 0 : valueForMoney]);
				basket.put(APP_CONSTANT.POPULARITY, popularityArr[popularity == 0 ? 0 : popularity]);
				...
				basket.put(APP_CONSTANT.TARGET_VALUE, SamcoHelper.commaFormat(Integer.toString(res.getInt(DBConstants.TARGET_VALUE))));
				basket.put(APP_CONSTANT.TARGET_INDEX, res.getInt(DBConstants.TARGET_INDEX));
				DateFormat targetDateFormat = new SimpleDateFormat("dd MMM,yyyy");
				basket.put(APP_CONSTANT.TARGET_DATE, targetDateFormat.format(res.getDate(DBConstants.TARGET_DATE)));
				basket.put(APP_CONSTANT.TARGET_INDEX_PERCENTAGE, res.getInt(DBConstants.TARGET_INDEX_PERCENTAGE));
		...
		resp.setEndFlag(totalBasketCount <= pageNo * 20);
	...
	public static SearchBasketResponse searchBasket(SearchBasketRequest searchReq) throws Exception {
		...
		try {
			...
			while (res.next()) {
				...
				// log.debug("indexList----->" + indexList);
				log.debug("First value is :: " + dFirstValue);
				log.debug("Second Value is :: " + dSecondValue);
				change = dSecondValue - dFirstValue;
				log.debug("change-------->" + change);
	...
	private static String getQuery(FetchBasketsRequest fetchReq) throws JSONException {
		...
		return QueryConstants.FETCH_BASKET_QUERY + filterQuery + priceFilterQuery + sortQuery + " limit 20 offset "
				+ offset;
	}
