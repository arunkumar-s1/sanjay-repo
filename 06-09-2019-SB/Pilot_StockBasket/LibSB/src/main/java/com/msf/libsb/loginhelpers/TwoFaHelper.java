...
public class TwoFaHelper
{
	...
	public void updateQuestionsInDB() throws Exception 
	{
		...
		try
		{
			...
			String sQuery = "delete from USERS_2FA where user_id = ?";
			ps = conn.prepareStatement(sQuery);
			ps.setString(1, this.sUserID);
			ps.execute();
			ps.close();

			sQuery = "insert into USERS_2FA (user_id, question, id, value) values (?,?,?,?)";
			ps = conn.prepareStatement(sQuery);
	...
	{		
		...
		try
		{
			String sQuery = "select id,answer from USERS_2FA where user_id = ?";
			conn = this.pool.getSamcoConnection();
			ps = conn.prepareStatement(sQuery);
			ps.setString(1, this.sUserID);
	...
	public JSONArray getQuestions() throws Exception
	{
		...
		try
		{
			String sQuery = "select value,question from USERS_2FA where user_id = ?";
	...
	public String getAnswersFromDB() throws Exception
	{
		...
		String sQuery = "select id,answer from USERS_2FA where user_id = ? and id in (" + sb.toString() + ")";
